...
s/>(seconds)</>(秒)</g
s/options: \[\['0', 'Default'],\['1','Manual']]/options: [['0', '預設'],['1','自訂']]/g
s/title: 'Use WAN port for LAN'/title: '將 WAN 埠用於 LAN'/g
...
s/title: 'Group Key Renewal'/title: '群組金鑰更新間隔'/g
s/title: 'Radius Server'/title: 'Radius 認證伺服器'/g
...
s/options: \[\['0','Link With...'],\['1','Automatic']]/options: [['0','指定連結...'],['1','自動連結']]/g
...
s/'Invalid hash or URL'/'無效的雜湊值(hash)或網址(URL)'/g
s/'DDNS update is running. Please refresh after a few seconds.'/'DDNS 更新中. 請稍後再重新整理.'/g
...
s/>\* This service determines the IP address using its own method.</>* 此服務商使用自己的方法來判定 IP.</g
...
s/'Invalid name. Only characters "A-Z 0-9 . - _" are allowed.'/'無效的名稱. 只允許 "A-Z 0-9 . - _" 等字元.'/g
...
s/>Tracking \/ NAT Helpers</>Tracking \/ NAT 輔助模組(Tracking \/ NAT Helpers)</g
s/>Miscellaneous</>其他設定(Miscellaneous)</g
s/title: 'TTL Adjust'/title: 'TTL 調整'/g
s/title: 'Inbound Layer 7'/title: '下載啟用應用層(Layer 7)過濾'/g
...
s/title'>Settings</title'>設定(Settings)</g
...
s/><small>(optional; ex: "/><small>(可空白不限定來源; 單一 WAN IP 或限定來源位址範圍; 例如: "/g
...
s/title: 'Reset class when changing settings'/title: '當變更設定, 重設等級'/g
...
s/>TCP Vegas <small>(network congestion control)</>TCP Vegas <small>(網路擁塞控管)</g
...
s/title: 'Blocked Resources'/title: '封鎖連線資源'/g
s/' Block All Internet Access'/' 封鎖所有的網際網路連線'/g
s/title: 'Port \/<br>Application'/title: '通訊埠 \/<br>應用程式'/g
s/title: 'HTTP Request'/title: '封鎖 HTTP 連線請求'/g
s/title: 'HTTP Requested Files'/title: '封鎖 HTTP 下載檔案'/g
...
s/('Frequent saving to NVRAM or JFFS2 is not recommended. Continue anyway?')/('不建議對 NVRAM 或 JFFS2 作頻繁的寫入儲存. 是否繼續執行?')/g
s/('Note: ' + path + ' will be treated as a file. If this is a directory, please use a trailing \/. Continue anyway?')/('備註: ' + path + ' 將會被視為一個檔案. 如果這是一個目錄, 路徑尾端請使用 \/. 是否繼續執行?')/g
...
s/\[9,'Every 9 Hours'],\[12,'Every 12 Hours'],\[24,'Every 24 Hours'],\[48,'Every 2 Days'],\[72,'Every 3 Days'],\[96,'Every 4 Days']/[9,'每 9 小時'],[12,'每 12 小時'],[24,'每天'],[48,'每二天'],[72,'每三天'],[96,'每四天']/g
...
s/>Startup LED</>開機 LED 指示燈(Startup LED)</g
...
s/'a few more seconds'/'數秒鐘'/g
s/'about ' : '') + ftime + ' second' + ((ftime == 1) ? '' : 's')/'約 ' : '') + ftime + ' 秒'/g
s/value="Format \/ Erase..."/value="格式化 \/ 清除..."/g
s/>Please wait for </>請稍候 </g
s/>about 60 seconds</>約 60 秒</g
...
s/title: 'Limit'/title: '限制'/g
s/<small>(messages per minute \/ 0 for unlimited)/<small>(每分鐘之訊息數量 \/ 0 不限制)/g
...
s/' script is too long. Maximum allowed is ' + x + ' bytes.'/' 指令太多. 最大允許 ' + x + ' 位元組.'/g
s/'> Word Wrap'/'> 自動斷行'/g
...
s/alert('Expecting a ".bin" or ".trx" file.')/alert('副檔名應為 ".bin" 或 ".trx".')/g
s/confirm('Are you sure you want to upgrade using ' + name + '?')/confirm('確定使用 ' + name + ' 升級韌體嗎?')/g
s/>Upgrade Firmware</>升級韌體(Upgrade Firmware)</g
s/Select the file to use:/選擇韌體檔案:/g
s/value='Upgrade'/value='升級'/g
s/>Current Version:</>目前版本:</g
s/>Free Memory:</>可用記憶體:</g
s/<small>(aprox. size that can be buffered completely in RAM)/<small>(aprox. 可用記憶體容量完全用於韌體升級緩衝區)/g
...
s/'You can now unplug the router.'/'您現在可以關閉路由器電源了.'/g
