...
sg.verifyFields = function(row, quiet)
{
	...
	if (!fip) {
		...
		if (isNaN(s) || (s <= 0) || (s >= 255)) {
			ferror.set(f[1], '不正确的 IP 地址', quiet);
			ok = 0;
		}
		if (this.inStatic(s)) {
			ferror.set(f[1], '重复的IP地址', quiet);
			ok = 0;
		}
		if (ok) f[1].value = s;
	}
	...
	if (s.length > 0) {
		if (s.search(/^[.a-zA-Z0-9_\- ]+$/) == -1) {
			ferror.set(f[2], '无效的名字,只能输入 "A-Z 0-9 . - _" are allowed.', quiet);
			ok = 0;
		}
		if (this.existName(s)) {
			ferror.set(f[2], '重复的名字.', quiet);
			ok = 0;
		}
		if (ok) f[2].value = s;
	}
...
sg.setup = function()
{
	...
	this.headerSet(['MAC 地址', 'IP 地址', '主机名称']);
...
<div class='section-title'>静态DHCP</div>
<div class='section'>
