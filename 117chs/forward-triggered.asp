...
tg.setup = function() {
	this.init('tg-grid', 'sort', 50, [
		...
		{ type: 'text', maxlen: 16 },
		{ type: 'text', maxlen: 32 }]);
	this.headerSet(['开启', '协定', '触发通讯埠', '转送到通讯埠', '叙述']);
...
<li>例: 巴哈姆特的 BBS 会使用 Port 113 做验证.<br>在本例中, 连往 BBS 所用的 Port 为 23, 接收验证的 Port 为 113, 所使用的通讯协议皆为 TCP.<br>所以我们要加入一条规则, 在 [协议] 中选中 [TCP]<br>[触发端口] 填入 [23]<br>[映射端口] 填入 [113]<br>最后记得要按 [增加] 与 [保存] 按钮<br>
<li>开启的通讯埠若未使用,几分钟之后会自动关闭.
</ul>
</div>
