...
dg.populate = function()
{
	...
	for (i = dhcpd_lease.length - 1; i >= 0; --i) {
		a = dhcpd_lease[i];
		e = get(a[2], a[1]);
		e.lease = '<small><a href="javascript:deleteLease(\'L' + i + '\',\'' + a[1] + '\')" title="删除DHCP租约" id="L' + i + '">' + a[3] + '</a></small>';
		e.name = a[0];
	...
	for (i = list.length - 1; i >= 0; --i) {
		...
		if ((e.ip.length == 0) || (e.ip == '-')) {
			...
			a = '<a href="javascript:addStatic(' + i + ')" title="加入至 静态DHCP">' + e.ip + '</a>';
		}

		if (e.mac.match(/^(..):(..):(..)/)) {
			b = "<a href='http://standards.ieee.org/cgi-bin/ouisearch?" + RegExp.$1 + "-" + RegExp.$2 + "-" + RegExp.$3 + "' target='_new' title='搜寻 OUI 码'>" + e.mac + "</a>";
		}
		else {
			b = '';
		}
...
dg.setup = function()
{
	...
	this.headerSet(['接口', 'MAC 地址', 'IP 地址', '名称', 'RSSI &nbsp; &nbsp; ', '品质', '剩余租约 &nbsp; &nbsp; ']);
	this.populate();
	this.sort(2);
}
...
<script type='text/javascript'>
if (nvram.wl_radio == '1') {
	W('<div style="float:left"><b>背景噪声:</b> <span id="noise">' + wlnoise + '</span> <small>dBm</small>');
	if ((nvram.wl_mode == 'ap') || (nvram.wl_mode == 'wds')) {
		W(' &nbsp; <input type="button" value="测量" onclick="javascript:window.location=\'wlmnoise.cgi?_http_id=' + nvram.http_id + '\'">');
	}
	W('</div>');
}
</script>
