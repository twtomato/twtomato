...
var abc = ['未分级', '最高级', '高级', '中级', '低级', '最低级', 'Ａ级','Ｂ级','Ｃ级','Ｄ级','Ｅ级'];
...
grid.setup = function() {
	this.init('grid', 'sort');
	this.headerSet(['协议', '来源', '源端口', '目的地', '目的端口', '类型']);
}
...
function init()
{
	...
	if (viewClass != -1) E('stitle').innerHTML = '分级连线内容 : ' + abc[viewClass];
	grid.setup();
...
<div class='section-title' id='stitle' onclick='document.location="qos-graphs.asp"' style='cursor:pointer'>详细内容</div>
<div class='section'>
<table id='grid' class='tomato-grid' style="float:left" cellspacing=1></table>
<input type='checkbox' id='resolve' onclick='resolveChanged()' onchange='resolveChanged()'> 自动解析IP
