...
cg.verifyFields = function(row, quiet) {
	...
	ferror.set(f, '不正确的 MAC 或 IP 位址/范围', quiet);
	return false;
}
...
function save()
{
	...
	if (E('_f_type').value == 0) {
		...
		if (e.value != 0) {
			a = cg.getAllData();
			if (a.length == 0) {
				ferror.set(e, 'MAC 错误 或 IP 位址 已被指定', 0);
				return;
			}
	...
	if (data.length > 2048) {
		alert('规则太大. 请减少 ' + (data.length - 2048) + ' 字元.');
		return;
	}
...
<div class='section-title'>网路浏览限制</div>
<div class='section'>
<script type='text/javascript'>
...
createFieldTable('', [
	{ title: '开启', name: 'f_enabled', type: 'checkbox', value: rule[1] == '1' },
	{ title: '注解', name: 'f_desc', type: 'text', maxlen: 32, size: 35, value: rule[9] },
	{ title: '排程', multi: [
	...
	{ title: '连线类型', name: 'f_type', type: 'select', options: [[0,'一般限制'],[1,'关闭无线']], value: (rule[5] == '~') ? 1 : 0 },
	{ title: '适用于', name: 'f_comp_all', type: 'select', options: [[0,'所有的电脑 / 装置'],[1,'以下列表...'],[2,'排除以下列表...']], value: 0 },
	{ title: '&nbsp;', text: '<table class="tomato-grid" cellspacing=1 id="res-comp-grid"></table>' },
	{ title: 'Blocked Resources', name: 'f_block_all', type: 'checkbox', suffix: ' 限制所有的网际网路连线', value: 0 },
	{ title: '连接埠 /<br>应用程式', indent: 2, text: '<table class="tomato-grid" cellspacing=1 id="res-bp-grid"></table>' },
	{ title: 'HTTP Request', indent: 2, name: 'f_block_http', type: 'textarea', value: rule[7] },
	{ title: '限制 HTTP 要求的档案', indent: 2, multi: [
