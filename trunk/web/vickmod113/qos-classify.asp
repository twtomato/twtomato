...
<title>[<% ident(); %>] 頻寬管理 QoS:分類機制</title>
...
var abc = ['最高等', '高等級', '中等級', '低等級', '最低', 'Ａ等級', 'Ｂ等級', 'Ｃ等級', 'Ｄ等級', 'Ｅ等級'];

var ipp2p = [
	[0,'IPP2P (關閉)'],[0xFFF,'All IPP2P filters'],[1,'AppleJuice'],[2,'Ares'],[4,'BitTorrent'],[8,'Direct Connect'],
...
qosg.setup = function() {
	var i, a, b;
	a = [[-2, '任何協定'],[-1,'TCP/UDP'],[6,'TCP'],[17,'UDP']];
	for (i = 0; i < 256; ++i) {
	...
	this.init('qg', 'move', 50, [
		{ multi: [
			{ type: 'select', options: [[0,'任何位址'],[1,'Dst IP'],[2,'Src IP'],[3,'Src MAC']],
...
<div class='section-title'>分類機制 上傳管理（對外）</div>
<div class='section'>
