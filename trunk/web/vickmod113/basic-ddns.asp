...
var services = [
	...
	['opendns', 'OpenDNS', 'http://www.opendns.com/', 'uoz'],
		...
		if (enabled) {
			...
			if (op.c) {
				...
				if ((!v_length('_f_user' + i, quiet, 1)) ||
					...
					((op.h) && (!v_length('_f_host' + i, quiet, 1))) ||
...
for (i = 0; i < 2; ++i) {
	...
	createFieldTable('', [
		...
		{ title: '帳號', name: 'f_user' + i, type: 'text', maxlen: 64, size: 35, value: u[0], hidden: 1 },
		{ title: '密碼', name: 'f_pass' + i, type: 'password', maxlen: 64, size: 35, value: u[1], hidden: 1 },
		{ title: '主機名稱', name: 'f_host' + i, type: 'text', maxlen: 96, size: 35, value: v[2], hidden: 1 },
