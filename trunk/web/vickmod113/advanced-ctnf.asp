...
<title>[<% ident(); %>] 進階設定: 連線追蹤 / 過濾模組</title>
...
function check()
{
	...
	checker.onCompleted = function(text, xml) {
		...
		for (i = 1; i < 13; ++i) {
			E('count' + i).innerHTML = '&nbsp; <small>('+ ((conntrack[i] || 0) * 1) + ' 個狀況符合)</small>';
		}
...
<br>
<input type='button' value='清除閒置' onclick='expireClicked()' id='expire'>
<br><br>
</div>
...
<div class='section-title'>Tracking / NAT 模組</div>
<div class='section'>
<script type='text/javascript'>
createFieldTable('', [
...
createFieldTable('', [
	{ title: '調整ＴＴＬ', name: 'nf_ttl', type: 'select', options: v, value: nvram.nf_ttl },
