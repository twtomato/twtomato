...
{
	if (changed) {
		if (!confirm("設定尚未儲存將會遺失. 是否繼續執行?")) return;
	}
...
function verifyFields(focused, quiet)
{
	...
	if ((a.value != 3) && (b != 0) && (a.value != b)) {
		ferror.set(a, '關閉 HTTP 和 HTTPS 將只能從遠端管理此機器', quiet);
		ok = 0;
	}
	else {
		ferror.clear(a);
...
function save()
{
	...
	if (a == 0) {
		if (!confirm('請注意：手動關閉 WebGUI 介面，若關閉、只能使用 Telnet 和 SSH 登入或從硬體 Reset 機器，您確定要這樣做嗎?')) return;
...
{ title: '風格', name: 'web_css', type: 'select',
		options: [['red','Tomato'],['black','Black'],['blue','Blue'],['bluegreen','Blue &amp; Green (Lighter)'],['bluegreen2','Blue &amp; Green (Darker)'],['cyan','Cyan'],['olive','Olive'],['pumpkin','Pumpkin'],['purple','Purple'],['enlightened','Enlightened'],['ext/custom','Custom (ext/custom.css)']], value: nvram.web_css },
	{ title: '顯示在工具列', name: 'f_favicon', type: 'checkbox', value:  nvram.web_favicon == 1 }
]);
</script>
</div>
...
<script type='text/javascript'>
createFieldTable('', [
	{ title: '開機時開啟', name: 'f_sshd_eas', type: 'checkbox', value: nvram.sshd_eas == 1 },
...
W('<input type="button" value="' + (sdup ? '關閉' : '開啟') + ' Now" onclick="toggle(\'sshd\', sdup)" id="_sshd_button">');
</script>
</div>
...
<script type='text/javascript'>
createFieldTable('', [
	{ title: '開機時開啟', name: 'f_telnetd_eas', type: 'checkbox', value: nvram.telnetd_eas == 1 },
	{ title: '通訊埠', name: 'telnetd_port', type: 'text', maxlen: 5, size: 7, value: nvram.telnetd_port }
]);
W('<input type="button" value="' + (tdup ? '關閉' : '開啟') + ' Now" onclick="toggle(\'telnetd\', tdup)" id="_telnetd_button">');
</script>
</div>
