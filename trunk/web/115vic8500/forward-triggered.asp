...
tg.setup = function() {
	this.init('tg-grid', 'sort', 50, [
		...
		{ type: 'text', maxlen: 16 },
		{ type: 'text', maxlen: 32 }]);
	this.headerSet(['開啟', '協定', '觸發通訊埠', '通訊埠轉發', '說明']);
...
<div class='section-title'>觸發式通訊埠轉發</div>
<div class='section'>
...
<li>一旦偵測到.
<li>觸發程式通訊埠，送往指定內傳通訊埠號的上傳封包便會轉向您的電腦.<li>例: 巴哈姆特的 BBS 會使用 Port 113 做驗證.<br>在本例中, 連往 BBS 所用的 Port 為 23, 接收驗證的 Port 為 113, 所使用的通訊協定皆為 TCP.<br>所以我們要加入一條規則, 在 [協定] 中選則 [TCP]<br>[觸發通訊埠] 填入 [23]<br>[轉送到通訊埠] 填入 [113]<br>最後記得要按 [新增] 與 [儲存] 鈕<br>
