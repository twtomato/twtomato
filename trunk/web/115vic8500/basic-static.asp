...
<title>[<% ident(); %>] 一般設定：DHCP MAC 綑綁 IP</title>
...
sg.verifyFields = function(row, quiet)
{
	...
	if (v_macz(f[0], quiet)) {
		if (this.existMAC(f[0].value)) {
			ferror.set(f[0], '重覆的 MAC 位址', quiet);
			ok = 0;
		}
	}
	else ok = 0;
	...
	if (s.length > 0) {
		if (s.search(/^[.a-zA-Z0-9_\- ]+$/) == -1) {
			ferror.set(f[2], '錯誤的字元組合. 僅接受. "A-Z 0-9 . - _" 等字元.', quiet);
			ok = 0;
		}
		if (this.existName(s)) {
...
<div class='section-title'>DHCP MAC 綑綁 IP</div>
<div class='section'>
