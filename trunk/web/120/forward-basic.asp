...
fog.setup = function() {
	this.init('fo-grid', 'sort', 50, [
		...
		{ type: 'text', maxlen: 15 },
		{ type: 'text', maxlen: 32 }]);
	this.headerSet(['開啟', '協定', '來源位址', '通訊埠範圍', '本地通訊埠', '本地IP', '敘述']);
...
<div>
<ul>
<li><b>來源位址</b> <i>(非必填)</i> - 轉發至設定的 IP範圍. 例: "1.2.3.4", "1.2.3.4 - 2.3.4.5", "1.2.3.0/24".
<li><b>通訊埠範圍</b> - 從 WAN對應進來的埠. 例: "2345", "200,300", "200-300,400".
<li><b>本地通訊埠</b> <i>(非必填)</i> -  .若未填上 本地通訊埠, 便自動對應 <i>通訊埠範圍</i>.
當 本地通訊埠 與 通訊埠範圍 不同時,才須填入 本地通訊埠.

<li><b>本地IP</b> - 對應區域網路(LAN) 內的 IP 位址.
</ul>
</div>
