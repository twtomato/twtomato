...
// advanced-buttons
	...
	{ "sesx_b3",			"4"				},
	{ "sesx_b4",			"4"				},
	{ "sesx_script", "service wan restart" },
	{ "sesx_script_1", "[ $1 -ge 20 ] && telnetd -p 233 -l /bin/sh\n" },
	{ "sesx_script_2", "led white off"				},
	{ "script_brau",
...
// admin-script
	{ "script_init",		""				},
	{ "script_shut",		""				},
	{ "script_fire",		"/usr/sbin/iptables -I POSTROUTING -t nat -o vlan1 -d 10.10.1.0/24 -j MASQUERADE\x0aip addr add 10.10.1.2/24 dev vlan1 brd +\x0a"				},
	{ "script_wanup",		"killall sesx_script.sh\x0aI=`ifconfig ppp0|grep -q \x27UP\x27`\x0awhile sleep 1; do\x0aif I=0; then\x0aXFER=`ifconfig ppp0|grep bytes`\x0aif [ \x22$XFER\x22 != \x22$PXFER\x22 ]; then\x0aled amber on\x0aPXFER=$XFER\x0aelse\x0aled amber off\x0afi\x0aelse\x0aled amber off\x0aled white off\x0afi\x0adone"				},
