...
static void sysinit(void)
{
	...
	hardware = check_hw_type();
#if TOMATO_N
	if (hardware == HW_BCM4702) modprobe("diag");
	modprobe("et");
#else
	...
	modprobe("wl");
#if TOMATO_SL
	if (hardware == HW_BCM4704_BCM5325F) {
		modprobe("ctmisc");				// EOU/SN/MAC? led?
	}
#endif
	modprobe("tomato_ct");
	...
	for (i = 0; i < 200; ++i) {
		f_read_string("/proc/usb/scan_done", s, sizeof(s));
		if (s[0] == '1') break;
		usleep(10000);
	}
	cprintf("usb scan done: i=%d\n", i);
#endif
...
int init_main(int argc, char *argv[])
{
	...
	for (;;) {
		...
		case START:
			...
			led(LED_DIAG, 0);
/*			
			syslog(LOG_INFO, "Model: %s (%s) / Type: %d / Features: %s",
				nvram_safe_get("t_model_name"), nvram_safe_get("t_model"),
				check_hw_type(), nvram_safe_get("t_features"));
*/
			state = IDLE;
