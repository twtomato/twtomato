...
{
	if (!confirm('删除 ' + data[4] + '? [' + data[0] + '->' + data[1] + ' ' + data[2] + ']')) return;
...
ug.setup = function() {
	...
	this.headerSet(['外部端口', '内部端口', '内部地址', '协议', '说明']);
	ug.populate();
}
...
<div class='section-title'>UPnP 清单</div>
<div class='section'>
...
<script type='text/javascript'>
createFieldTable('', [
	{ title: '启用 UPnP', name: 'f_upnp_enable', type: 'checkbox', value: (nvram.upnp_enable == '1') },
	{ title: '在网络中显示',  name: 'f_upnp_mnp',  type: 'checkbox',  value: (nvram.upnp_mnp == '1') }
]);
</script>
</div>
