...
function msgLoc(s)
{
	...
	if (r = s.match(/^(.*?): (.*)/)) {
		r[2] = r[2].replace(/#RETRY (\d+) (\d+)/,
			function(s, min, num) {
				return '<br><small>(' + ((num >= 1) ? (num + '/3: ') : '') + '自动重试于 ' + min + ' 分钟内)</small>';
			}
		);
...
for (i = 0; i < 2; ++i) {
	...
	createFieldTable('', [
		...
		{ title: '密码', name: 'f_pass' + i, type: 'password', maxlen: 64, size: 35, value: u[1], hidden: 1 },
		...
		{ title: ' ', text: '(使用 @IP 给当前的IP地址使用)', rid: ('custmsg' + i), hidden: 1 },
