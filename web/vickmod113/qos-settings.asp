...
classNames = ['最高等', '高等級', '中等級', '低等級', '最低', 'Ａ等級', 'Ｂ等級', 'Ｃ等級', 'Ｄ等級', 'Ｅ等級'];
...
f = [];
f.push({ title: '最大上傳速率', name: 'qos_obw', type: 'text', maxlen: 6, size: 8, suffix: ' kbit/s', value: nvram.qos_obw });
f.push(null);
j = 0;
for (i = 0; i < 10; ++i) {
	x = cc[j++] || 1;
...
f = [];
f.push({ title: '最大下載速率', name: 'qos_ibw', type: 'text', maxlen: 6, size: 8, suffix: ' kbit/s', value: nvram.qos_ibw });
f.push(null);
for (i = 0; i < 10; ++i) {
	f.push({ title: classNames[i], multi: [
