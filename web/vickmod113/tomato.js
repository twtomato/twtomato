...
//			['Watchdog',		'watchdog.asp'],
	...
	for (i = 0; i < menu.length; ++i) {
		...
		if (m[1] == 'logout.asp') {
			// can't logout in IE7...
			try {
				if ((navigator.userAgent.match(/MSIE\s+(\d+)/)) && ((RegExp.$1 * 1) >= 7)) {
					continue;
				}
			}
			catch (ex) {
...
function reboot()
{
	if (confirm("重新開機?")) form.submitHidden('tomato.cgi', { _reboot: 1, _commit: 0, _nvset: 0 });
}

function shutdown()
{
	if (confirm("關機?")) document.location = 'shutdown';
}
