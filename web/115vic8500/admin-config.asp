...
function resetButton()
{
	...
	if (i == 0) return;
	if ((i == 2) && (features('!nve'))) {
		if (!confirm('請注意: 清除 NVRAM 的 ' + nvram.t_model_name + ' 路由器可能會損毀.完全清除之後 可能無法正確設定 NVRAM. 是否繼續執行 ?')) return;
	}
	if (!confirm('確定嗎 ?')) return;
...
<div class='section'>
	...
	<select name='mode' id='restore-mode'>
		...
		<option value=1>恢復TOMATO預設值 (一般)</option>
		<option value=2>清除NVRAM全部資料 (徹底清除)</option>
	</select>
